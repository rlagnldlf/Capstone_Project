...
학습 데이터가 모델 성능에 큰 영향을 미친다는 것을 알게 되었다. 각 게임당 1000건씩 총 3000건을 균일하게 학습시킨 첫 번째 학습 데이터는 균형 잡힌 데이터로 안정적인 학습 과정을 거쳤다. 반면, 원본 데이터 비율에 맞춰 3000건을 학습시킨 두 번째 학습 데이터는 초기에는 손실값이 더 낮았으나 데이터의 불균형으로 인해 과적합 징후를 보였다. 결과적으로 첫 번째 모델이 더 나은 성능과 안정성을 보여주었다. 이를 통해 학습 데이터의 균형이 중요하며, 초기에는 균형 잡힌 학습 데이터의 정확도가 떨어지더라도 지속적인 학습을 통해 성능이 개선될 수 있다는 것을 알게 되었다. 또한, 데이터를 학습시킬 때 과적합을 방지하기 위한 다양한 기법의 필요성을 느꼈다. 이 프로젝트의 개선점으로는 같은 장르의 다양한 게임 리뷰 데이터를 추가로 수집하고, 과적합을 방지하는 기법을 적용하여 학습시키면 더욱 높은 정확도를 얻을 수 있을 것이다.
